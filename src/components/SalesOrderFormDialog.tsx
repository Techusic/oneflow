...
import { SalesOrder } from "@/data/staticData";
...
interface SalesOrderFormDialogProps {
  ...
  projectId: string;
  onSave: (data: SalesOrderFormValues) => void;
}
...
export function SalesOrderFormDialog({ open, onOpenChange, salesOrder, projectId, onSave }: SalesOrderFormDialogProps) {
  ...
  const onSubmit = (data: SalesOrderFormValues) => {
    const finalData = {
      ...data,
      amount: total || data.amount, // Use calculated total if order lines exist
    };
    onSave(finalData);
    onOpenChange(false);
    form.reset();
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="SO01" {...field} readOnly className="bg-muted" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
            ...
            <DialogFooter className="flex justify-between">
              ...
              <div className="flex gap-2">
                <Button type="submit" variant="outline">
                  {salesOrder ? "Save Changes" : "Create SO"}
                </Button>
                {!salesOrder && (
